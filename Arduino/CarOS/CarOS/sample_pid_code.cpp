...
ISR(ADC_vect) // SenseI sets this up so it will be entered 125,000 times per second.
{
	...
	int ADCvalue = ADCL;
	ADCvalue += (ADCH << 8); // 0 to 1023. Reading ADCH causes update... so read ADCL first
	switch (toggler)
	{
...
public:
	...
	void begin()
	{
		pinMode(OCD, INPUT_PULLUP);
		setupAtoD();
	}

	inline bool alarmed()
	{
		return !digitalRead(OCD);
	}

	// If is pulled low then there's an error
	inline long milliAmps()
	{
		return map(batteryAverage.getValue(), IZERO, 1023, 0, range);
	}
...
public:
	...
	inline long demand()
	{
		return constrain(map(ADC_thumb, THUMB_MIN, THUMB_MAX, 0, IMAXDEMAND), 0, IMAXDEMAND);
	}
};
...
public:
	...
	inline void drive(int perMille)
	{
		...
		if (perMille)
		{
			idleTimer.trigger(); // Keep the timer alive
		}
